...
class HeartForYou:
    def __init__(self, horizontal_scale=8, measurement_time_s=30, frequency=250, scale_koeff=0.5, offset=12):
        self.sample_number = 0
        self.horizontal_scale = horizontal_scale
        self.measurement_time_s = measurement_time_s  # user's choice
    ...
    def show_welcome_screen(self):
        self.oled.fill_rect(75, 10, 6, 34, 1)
        self.oled.fill_rect(60, 10, 6, 15, 1)
        self.oled.fill_rect(60, 22, 20, 4, 1)
        self.oled.text('HEART ', 30, 32)
        self.oled.text('U ', 86, 32)
        self.oled.show()
        sleep(5)
    ...
    def hrv_analysis_menu(self):
        ...
        mean_ppi = max(int(sum(self.filtered_ppi) / max(len(self.filtered_ppi), 1)), 1)
    ...
    def show_measurement_result(self, menu_add):
        ...
        while not exit_loop:
            self.menu_items = [self.measurement_result["date"]] + [f"{key} {value}" for key, value in
                                                                   self.measurement_result.items() if
                                                                   value != 0 and key != "date"] + menu_add
            selected_item = self.select_menu_item()
    ...
    def kubious(self):
        ...
        try: 
            ...
            print(e)
            self.display_info_on_oled("... no connection")
            self.clean_up()
            self.show_main_user_menu()
    ...
    def measurement_start(self):
        time_mapping = {
            ...
            '1 min': 60,
            '3 min': 180}
        self.menu_items = ['30 sec', '1 min', '3 min']
        self.selected_menu_item = 0
    ...
    def measurement(self):
        ...
        while utime.ticks_diff(utime.ticks_ms(),
                               start_time) < self.measurement_time_s * 1000 and not stop_button_pressed:
            ...
            if self.sample_number > self.normalisation_window:
                ...
                normalized_value = max(0,
                                       min((average_sample - scale_min_value_) / scale,
                                           1))
                #gc.collect()
                ...
                if current_gradient < 0 <= previous_gradient:
                    ...
                    peak_threshold_condition = self.normalized_values[current_peak] > (
                                self.peak_threshold * last_rpeak_value)
    ...
    def draw(self):
        ...
        if self.current_bpm > self.max_bpm or self.current_bpm < self.min_bpm:
            ...
            string_bpm = str(self.current_bpm)
            self.oled.text("BPM " + string_bpm, self.screen_width // 5, self.screen_height - 10)
        sample_x = 1
        for i in range(max(0, len(self.normalized_values) - self.screen_width // 3), len(self.normalized_values),
                           1):
                ...
                if i in self.r_peaks:
                    self.oled.line(sample_x, 0, sample_x, self.screen_height // 2, 1)
                self.oled.line(sample_x - 3, previous_y, sample_x, sample_y, 1)
                previous_y = sample_y
                sample_x += 3
        # show stop button
        self.oled.fill_rect(int(self.screen_width * 7 / 10), int(self.screen_height * 10 / 12) - 1, 40, 13, 1)
        self.oled.text("STOP", int(self.screen_width * 7 / 10) + 5, self.screen_height - 10, 0)
        self.oled.show()
    ...
    def select_menu_item(self):
        ...
        while True:
            self.oled.fill(0)
            display_start = max(0, self.selected_menu_item - 2)
